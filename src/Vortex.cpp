...
Vortex::Vortex()
	...
	, texture(0)
{
	shading.color = VORTEX_COLOR;
	shading.attenuation_coeffs = VORTEX_ATTENUATION;
	shading.default_size = VORTEX_DEFAULT_SIZE;
	shading.ttl = 0.0f;
}


void Vortex::setup_gpu_data() {
...
void Vortex::setup(const std::string& shader_vert, const std::string& shader_frag, const std::string& texture_file)
{
...
void Vortex::initialize_dynamics(int _num_points, const vcl::vec3* pos, const vcl::vec3& speed, float ttl) {
	pos_to_follow = pos;
...
void Vortex::update_smoke_particle(Particle& particle, float dt) {
	...
	if (std::abs(x) > max_radius) x *= max_radius / std::abs(x);
	if (std::abs(y) > max_radius) y *= max_radius / std::abs(y);
...
void Vortex::modulate_smoke_particle_reset(Particle& particle) {
